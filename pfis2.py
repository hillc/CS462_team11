...
#reads lines of file je.txt to generate a list of words that aren't being indexed.
def stopwords():
    debug('stopwords')
    f = open('je.txt')
    for line in f:
...
'''always called. generates list of stopwords which is generated by the stopwords function by reading a je.txt file that lists words that aren't being indexed.'''
if len(stop) == 0:
    stopwords()

'''returns a list of words in the string passed that are separated by camelcase as long as the words are not in 'stop' which is generated by the stopwords function by reading a je.txt file that lists words that aren't being indexed. example: string = 'indexCamelWords' return = [(word, index),(word,camel),(word, words)] '''
...
'''Returns a list of words in the string passed in as long as the words are not in 'stop' which is generated by the stopwords function by reading a je.txt file that lists words that aren't being indexed.'''
...
#returns if they are different methods, classes, or packages after navigation. 
